...
int main()
{
    ...
    if (i) printf("元素%d的位置为:%d\n", x, i);
    else printf("未找到元素%d\n", x);
    printf("第二个顺序表\n");
    SqList mylist2;
    InitList_SqList(mylist2);
    printf("准备输入多少个数据?:");
    scanf_s("%d", &n);
    printf("开始输入%d个数据:\n", n);
    for (i = 1; i <= n; i++)
    ...
    printf("输入要查询的顺序: ");
    scanf_s("%d", &i);
    if (ListGetElem_SqList(mylist2, i, &e) == OK)
        printf("第%d个元素为:%d:\n", i, e);
    else
        printf("输入顺序有误\n");
    printf("输入要查询的元素: ");
    scanf_s("%d", &x);
    i = LocateElem_SqList(mylist2, x);
    if (i) printf("元素%d的位置为:%d\n", x, i);
    else printf("未找到元素%d\n", x);
    printf("顺序表合并:\n");
    SqList mylist3;
    MergeList_SqList(mylist1, mylist2, mylist3);
    for (int i = 0; i < mylist3.length; i++)
    {
        printf("%d ", mylist3.elem[i]);
    }
    return 0;
}