...
int main()
{
    ...
    printf("第二个\n请输入5个字符（不需要空格间隔，回车两次确认）：\n");
    CreateList_LinkList(mylist2, 5);
    while (getchar() != '\n'); // 清空缓冲区
    printf("链表内容为 \n");
    ListTravel_LinkList(mylist2);
    printf("\n\n请输入插入的字符: ");
    scanf_s("%c", &n);
    printf("输入插入的位置: ");
    scanf_s("%d", &i);
    ListInsert_LinkList(mylist2, i, n);
    printf("\n插入一个字符%c后，链表的内容为: \n", n);
    ListTravel_LinkList(mylist2);
    printf("\n\n要删除第几个字符?: ");
    scanf_s("%d", &i);
    ListDelete_LinkList(mylist2, i);
    printf("删除第%d个元素后,链表的内容为: \n", i);
    ListTravel_LinkList(mylist2);
    printf("\n\n");
    printf("\n\n要查询第几个字符: ");
    scanf_s("%d", &i);
    GetElem_LinkList(mylist2, i, n);
    printf("单链表的第%d个字符是%c。\n", i, n);
    LinkList mylist3;
    MergeList_LinkList(mylist1, mylist2, mylist3);
    printf("\n合并后的链表内容为: \n");
    ListTravel_LinkList(mylist3);
    while (mylist3)
    {
        LinkList p = mylist3;
